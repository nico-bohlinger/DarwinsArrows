function PopulationV3(){
    ...
    this.selection = function(){
        ...
        for(var i = 0; i < int(this.popsize * 0.25); i++){
            newArrows.push(new ArrowV3(this.pool[i][1]));
            this.pool[i] = null;
        }
        if(newArrows.length == 0){
            newArrows.push(new ArrowV3(this.pool[0][1]));
        }
        ...
        if(newArrows.length != this.popsize){
            for(var i = 0; i < int(this.popsize * 0.1); i++){
                while(true){
                    ...
                    if(randomOne){
                        newArrows.push(new ArrowV3(randomOne[1]));
                        break;
                    }
                }
            }
        ...
        for(var i = newArrows.length; i < this.popsize; i++){
            ...
            child.mutation();
            newArrows.push(new ArrowV3(child));
        }
